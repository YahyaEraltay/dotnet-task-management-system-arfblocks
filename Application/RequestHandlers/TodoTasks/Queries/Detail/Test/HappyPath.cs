namespace Application.RequestHandlers.TodoTasks.Queries.Detail.Test;
...
public class HappyPath : IArfBlocksTest
{
    ...
    {
        var requestPayload = new Application.RequestHandlers.TodoTasks.Queries.Detail.RequestModel()
        {
            Id = todoTask.Id,
        };

        var requestOperator = new ArfBlocksRequestOperator(_dependencyProvider);
        var response = await requestOperator.OperateInternalRequest<Application.RequestHandlers.TodoTasks.Queries.Detail.Handler>(requestPayload);
        if (response.HasError)
            throw new Exception(response.Error.Message);

        response.HasError.Should().Be(false);

        var responsePayload = (Application.RequestHandlers.TodoTasks.Queries.Detail.ResponseModel)response.Payload;
